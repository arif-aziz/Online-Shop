...
class ListingController extends BaseController {
    ...
    {
    	$listing   = Listing::where('listing_permalink', $permalink)->firstOrFail();
        //$image     = Image::where('id_listing', $listing->id_listing)->get();
        $nama_kota = Area::where('id_kota', $listing->store->id_kota)
                        ->where('id_prov', 35)
                        ->where('level', 2)
                        ->pluck('nama');
